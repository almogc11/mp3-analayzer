...
class AudioFFTAnalyzer:
    ...
    def load_mp3(self, file_path=None):
        ...
        try:
            ...
            self.audio, self.sample_rate = librosa.load(file_path, sr=None, mono=True)
            print(f"Loaded: {file_path}")
            print(f"Duration: {len(self.audio) / self.sample_rate:.2f} seconds")
            print(f"Original sample rate: {self.sample_rate} Hz")
            print(f"Total samples: {len(self.audio)}")
            return True
    ...
    def resample_audio(self, target_sample_rate=1000):
        ...
        try:
            ...
            if self.sample_rate != target_sample_rate:
                self.samples = librosa.resample(self.audio, orig_sr=self.sample_rate, target_sr=target_sample_rate)
                self.sample_rate = target_sample_rate
            else:
                self.samples = self.audio.copy()
    ...
    def analyze_mp3(self, file_path=None, target_sample_rate=1000, window_size=1024, save_plot=False):
        ...
        print("=== Audio FFT Analyzer (Librosa Version) ===\n")
...
def main():
    ...
    file_path = None  # Change this to your audio file path if desired
